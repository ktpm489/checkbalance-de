...
async function getUsedChains(address) {
  ...
  try {
    const url = `https://api.debank.com/user/used_chains?id=${address}&project_id=etherdelta`;
    const headers = {
      accept: "*/*",
...
async function getWalletUsdValueForChain(address, chain) {
  ...
  try {
    const url = `https://api.debank.com/token/balance_list?is_all=false&user_addr=${address}&chain=${chain}&project_id=etherdelta`;
    const headers = {
      accept: "*/*",
...
async function processWallets(inputFile, outputFile) {
  ...
  try {
    ...
    if (ethers.utils.isAddress(firstLine)) {
      ...
      var n = prompt("How many address you want to generate per mnemonic?: ");
  ...
  for (let i = 0; i < wallets.length; i++) {
    ...
    actions.push(async () => {
      try {
        ...
        if (balance > 0) {
          console.log(address, `$${parseFloat(balance).toFixed(2)}`.green);
        } else {
          console.log(address, `$${balance}`.gray);
        }

        fs.appendFileSync(options.output, `${address},${balance}\n`, "utf-8");
      } catch (error) {
        console.error("Error fetching wallet USD value address:", address);
        // console.error(error);
      }
    });
  }
