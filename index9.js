...
async function scrapeDebankProfile(address, browser, retryCount = 0, maxRetries = 3) {
  ...
  try {
    ...
    const pageLoadDelay = getRandomDelay(15000, 35000); // Increased delay
    ...
    if (retryCount < maxRetries) {
      ...
      await new Promise((resolve) => setTimeout(resolve, 5000));
      return scrapeDebankProfile(address, browser, retryCount + 1, maxRetries);
    }
  ...
  try {
    ...
    for (let i = 0; i < addresses.length; i++) {
      ...
      if (i < addresses.length - 1) {
        const requestDelay = getRandomDelay(8000, 20000); // Increased delay
