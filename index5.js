...
// Get random delay with more human-like variation
function getRandomDelay(min, max) {
  // Use exponential distribution for more realistic timing
  const range = max - min;
  const random = Math.random();
  
  // 70% of times will be in the lower half of the range (faster)
  // 30% of times will be in the upper half (slower, like distracted users)
  if (random < 0.7) {
  ...
  try {
    ...
    for (let i = 0; i < addresses.length; i++) {
      ...
      if (i < addresses.length - 1) {
        const requestDelay = getRandomDelay(3000, 10000);
